...
class CarlaAckermannControl(CompatibleNode):
    ...
    def control_speed(self, target_linear_velocity_abs, current_velocity_abs):
        max_throttle_proportional = 1
        max_brake_proportional = 1
        holy_p = 3.0

        velocity_change_required = target_linear_velocity_abs - current_velocity_abs
        throttle, brake = 0, 0
        if target_linear_velocity_abs < 0.1:
            return 0., 1.
        if velocity_change_required > 0.1:
            throttle, brake = min(holy_p * velocity_change_required / target_linear_velocity_abs, max_throttle_proportional), 0.0
            
        elif velocity_change_required < -0.1:
            throttle, brake = 0.0, min(- 1 * holy_p * velocity_change_required / target_linear_velocity_abs, max_brake_proportional)
        return throttle , brake 

    def control_speed_pid(self):
        self.speed_controller.auto_mode = True
        ...
        throttle, brake = 0, 0
        if self.info.target.speed_abs < 1e-6:
            return 0., 1.

        if self.info.status.speed_control_accel_target > 0:
            throttle = self.info.status.speed_control_accel_target / clipping_upper_border
        else:
            brake = self.info.status.speed_control_accel_target / clipping_lower_border

        print("control_speed_pid 4",throttle,brake)
        return throttle , brake
    ...
    def vehicle_control_cycle(self):
        ...
        # throttle_, brake_ = self.control_speed(self.info.target.speed_abs, self.info.current.speed_abs)
        throttle_, brake_ = self.control_speed_pid()
        if not self.info.output.hand_brake:
            # self.update_drive_vehicle_control_command()
            self.info.output.throttle = throttle_
            self.info.output.brake = brake_
    ...
    def update_drive_vehicle_control_command(self):
        ...
        if self.info.target.speed_abs < 0.10:
            self.info.output.brake = 1.
            self.info.output.throttle = 0.
    ...
    def update_current_values(self):
        ...
        if self.vehicle_status.control.reverse:
